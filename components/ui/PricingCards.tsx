    ...
    <div data-cta={cta}>
      <div className="grid grid-cols-1 items-stretch gap-6 md:grid-cols-3 md:gap-5 lg:gap-6">
        {planos.map((plano) => {
          ...
          const waLink = `https://wa.me/${italev.whatsapp}?text=${encodeURIComponent(`Olá! Tenho interesse no plano ${plano.nome} e gostaria de solicitar uma demonstração gratuita.`)}`;
          return (
            <article key={plano.id} aria-label={`Plano ${plano.nome}`} className={`relative flex flex-col rounded-[28px] p-6 transition-all duration-300 sm:p-8 lg:p-9 ${isPro ? 'z-10 border-2 border-[#FF5A00] shadow-[0_0_48px_rgba(255,90,0,0.15)] md:-translate-y-2' : 'border border-white/[0.08] hover:border-[#FF5A00]/30'}`} style={{ background: isPro ? 'linear-gradient(135deg, rgba(255,90,0,0.08) 0%, rgba(255,255,255,0.03) 100%)' : 'rgba(255,255,255,0.03)' }}>
              ...
              <div className="mb-2 mt-2">
                ...
                <p className="mt-4 rounded-xl border border-white/10 bg-black/20 px-3 py-2 text-sm font-semibold leading-relaxed text-[#F8FAFC]">Implantação única: {plano.implantacaoPrefixo && 'a partir de '}R${plano.implantacao.toLocaleString('pt-BR')}</p>
                {plano.implantacaoGratisAnual && <span className="mt-2 inline-flex w-fit rounded-full border border-emerald-400/30 bg-emerald-400/10 px-3 py-1 text-xs font-bold text-emerald-300">Grátis no plano anual</span>}
              </div>
